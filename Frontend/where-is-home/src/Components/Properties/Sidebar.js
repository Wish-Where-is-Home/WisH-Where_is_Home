...
import { faBed, faBathtub, faWarehouse, faSquareFull, faMapLocationDot } from '@fortawesome/free-solid-svg-icons';
import { useTranslation } from 'react-i18next';
import { useNavigate } from 'react-router-dom';
import imovel1 from './imovel1.png';

function Sidebar({ darkMode, isOpen, toggleSidebar, properties, openModal, fetchImageURLsImoveis }) {
    ...
    useEffect(() => {
        const fetchImages = async () => {
            ...
            for (let property of properties) {
                try {
                    const urls = await fetchImageURLsImoveis(property.id);
                    newImageURLs[property.id] = urls.length > 0 ? urls[0] : imovel1;
                } catch (error) {
                    newImageURLs[property.id] = imovel1;
                }
            }
        ...
        fetchImages();
    }, [properties, fetchImageURLsImoveis]);
                    ...
                    <div className="box-container" key={index} id={`property-${box.id}`} onClick={() => handlePropertyClick(box.id)}>
                        ...
                        <div className="box">
                            ...
                            <div className="photos">
                                <img className="imovel-img" src={imageURLs[box.id] || imovel1} alt={`Imovel ${box.id}`} />
                            </div>
                        </div>
                    </div>
                ))}
