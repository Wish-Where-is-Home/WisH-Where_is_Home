...
function OwnerPage({ darkMode }) {
    ...
    const handleRoomAvailabilityChange = (roomId, newAvailability) => {
        setAllRooms(prevRooms => prevRooms.map(room => room.id === roomId ? { ...room, disponivel: newAvailability } : room));
    };

    const handleDeleteProperty = (propertyId) => {
        setProperties(prevProperties => prevProperties.filter(property => property.id !== propertyId));
        setAllRooms(prevRooms => prevRooms.filter(room => room.property_id !== propertyId));
    };
            ...
            <div className='properties-container-owner'>
                ...
                <div className='o-show-properties'>
                    {properties.length > 0 && properties.map(property => (
                        <PropertyDetails key={property.id} property={property} allRooms={allRooms} token={token}
